extends CogAttack
class_name CogAttackProxy

const PROXY_BOOST_REFERENCE := preload("res://objects/battle/battle_resources/status_effects/resources/status_effect_mod_cog.tres")
...
func action() -> void:
	...
	tween.tween_callback(cog.set_animation.bind('slip-forward')) #'was 'slip-forward
	...
	tween.tween_callback(manager.battle_text.bind(cog, "+1 Proxy ability", BattleText.colors.orange[0], BattleText.colors.orange[1]))
...
func apply_boost(cog : Cog) -> void:
	var new_boost := PROXY_BOOST_REFERENCE.duplicate()
